...
const _grindPositive = (negativeRangesVariations, usedRanges) => {
	// Wee-ha! We have about tens of thousands entries at this time
	// It could take a long time
	const Range = require('./range').Range;
	const retVal = [];
	for (let negativeRanges of negativeRangesVariations) {
		// Redeem chars marked as used
		let redeemedRanges = subtractRanges(negativeRanges, usedRanges);
		let boundaries = [];

		for (let range of redeemedRanges) {
			boundaries.push({
				left: _getAlternativeBoundaries(range.from, range.to),
				right: _getAlternativeBoundaries(range.to, range.from)
			})
		}
	}
};
